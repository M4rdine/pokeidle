...
2. **Jogador** por modo:
   ...
   - `returning`: anda até tile adjacente ao `pokecenter`; ao chegar, `healing` com
     `healingUntilTick = tick + HEAL_TICKS`. Evento `returning` ao entrar no modo.
...
4. **Consequências**:
   - Selvagem com `hp <= 0`: `xpOnDefeat` para treinador e ativo; `levelFromXp` pode subir vários
     níveis (evento `levelUp` por nível, `hpMax` recalculado mantendo a proporção de HP);
   ...
   - Ativo com `hp / hpMax * 100 < returnHpPercent` e modo `fighting` ou `searching`: se houver
     poção no inventário, usa a mais fraca que cure (evento `itemUsed`); senão modo `returning`.
...
`createHuntState({ hunt, team, inventory, settings, seen }, deps)`: posição no `spawnPoint`, modo
`searching`, `tick 0`, todos os spawns já gerados (`count` selvagens cada), `nextWildId` sequencial.
`resumeHuntState` não existe: o estado serializado é o próprio estado.
...
- Unitários de `grid.ts` (caminho, bloqueio, sem caminho), de cada transição de modo em mapas 5x5
  sintéticos com registro mínimo, de captura (aplicabilidade, consumo de bola, sucesso/falha), de
  progressão (level up em cascata, evolução, loot).
