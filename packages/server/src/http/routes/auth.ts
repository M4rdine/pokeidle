...
import { userDto } from '../../account/dto.js'
...
export const authRoutes: FastifyPluginAsync<RouteDeps> = async (app, { db, config, now, realtime, registry }) => {
  ...
  app.post('/auth/register', AUTH_LIMIT, async (request, reply) => {
    ...
    return reply.status(201).send({ user: userDto(user), trainer: await trainerView(db, registry, trainer) })
  })
  ...
  app.post('/auth/login', AUTH_LIMIT, async (request, reply) => {
    ...
    reply.setCookie(SESSION_COOKIE, token, sessionCookieOptions(config.COOKIE_SECURE))
    return reply.send({ user: userDto(user), trainer: await trainerView(db, registry, trainer) })
  })
